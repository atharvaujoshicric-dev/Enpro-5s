...
const DEMO_USERS = [
  ...
  { email:"ceo@enpro.com",      password:"Ceo@123",     role:"ceo",          name:"CEO",            zone:null },
...
const AppState = {
  ...
  _get(k, def) { try { const v = localStorage.getItem(k); return v !== null ? JSON.parse(v) : def; } catch(e) { return def; } },
  ...
  restoreSession() {
    try { const s = sessionStorage.getItem("5s_sess"); if (s) this.currentUser = JSON.parse(s); } catch(e) {}
  },

  // Ping only specific user by email
  pingUser(toEmail, message, type, recordId) {
    const toUser = this.users.find(u => u.email === toEmail);
    if (!toUser) return;
    const p = this.pings;
    p.push({
      id: genId(), toEmail, toRole: toUser.role, toZone: toUser.zone,
      toName: toUser.name, message, type, recordId: recordId || null,
      createdAt: new Date().toISOString(), read: false
    });
    this.pings = p;
  },

  // Ping all users of a specific role+zone combo
  pingRole(toRole, toZone, message, type, recordId) {
    const targets = this.users.filter(u =>
      u.role === toRole && (!toZone || u.zone === toZone)
    );
    targets.forEach(u => this.pingUser(u.email, message, type, recordId));
  },

  // Ping all CEOs
  pingCEO(message, type, recordId) {
    this.pingRole("ceo", null, message, type, recordId);
  },
  ...
  getZoneManager(zoneName) {
    // returns first manager assigned to this zone
    return this.users.find(u => u.role === "zone_manager" && u.zone === zoneName) || null;
  }
};
